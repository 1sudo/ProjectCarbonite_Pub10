...
void BountyMissionObjectiveImplementation::activate()
{
	...
	if (isPlayerTarget())
	{
		...
		if (missionManager == NULL || mission == NULL || !missionManager->hasPlayerBountyTargetInList(mission->getTargetObjectId()) || !missionManager->hasBountyHunterInPlayerBounty(mission->getTargetObjectId(), getPlayerOwner()->getObjectID()) || !addPlayerTargetObservers())
		{
...
void BountyMissionObjectiveImplementation::complete()
{
	...
	//Award bountyhunter xp. (5x exp [from /50 -> /10])
...
void BountyMissionObjectiveImplementation::handlePlayerKilled(ManagedObject *arg1)
{
	...
	if (owner != NULL && killer != NULL && !completedMission)
	{
		if (owner->getObjectID() == killer->getObjectID())
		{
			...
			if (zoneServer != NULL)
			{
				...
				if (target != NULL)
				{
					...
					zBroadcast << "\\#66b3ff"
							   << "Bounty Hunter Guild Broadcast: \\#ffffff Guild member \\#ff0000" << hunterName << "\\#ffffff has claimed the Bounty on \\#ffd700" << targetName << "\\#ffffff.";
		...
		else if (mission->getTargetObjectId() == killer->getObjectID() || (npcTarget != NULL && npcTarget->getObjectID() == killer->getObjectID()))
		{
			// Get Bounty Hunter's Name
			String hunterName = owner->getFirstName();
			String huntedName = killer->getFirstName();

			// Fail the mission for the BH
			owner->sendSystemMessage("@mission/mission_generic:failed"); // Mission failed

			// Message to Jedi that defended themselves successfully.
			...
			// Death Broadcast to entire Galaxy
			StringBuffer zBroadcast;
			...
			// Setup music to play for the Jedi upon victory!
			PlayMusicMessage* pmm = new PlayMusicMessage("sound/music_themequest_victory_imperial.snd");
