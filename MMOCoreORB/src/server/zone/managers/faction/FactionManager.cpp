...
void FactionManager::awardPvpFactionPoints(TangibleObject *killer, CreatureObject *destructedObject)
{
	if (killer->isPlayerCreature() && destructedObject->isPlayerCreature())
	{
		...
		if (Zone *zone = ghost->getZone())
		{
			...
			// Fight Clubbing
			const String fightClubMessage = "Fight Clubbing is not tolerated within the FRS/GCW.";
			bool isFightClubbing = ghost->getAccountID() == killedGhost->getAccountID();
			if (isFightClubbing)
			{
				...
				UnicodeString message = killerCreature->getFirstName() + " tried to Fight Club. No experience awarded. Shame!";
				BaseMessage *msg = new ChatRoomMessage("[fight clubbing detected]", ServerCore::getInstance()->getZoneServer()->getGalaxyName(), message, pvpNotificationChat->getRoomID());
				pvpNotificationChat->broadcastMessage(msg);

				// Do nothing else since the pvp kill was invalid
				return;
			}
					...
					if (killerCreature->isInRange(groupMember, Range))
					{
						...
						if (killerCreature->isInRange(groupMember, Range))
						{
							...
							if (isFightClubbing)
							{
								...
								UnicodeString message = groupMemberCreature->getFirstName() + " tried to Fight Club. No experience awarded. Shame!";
								BaseMessage *msg = new ChatRoomMessage("[group-fight clubbing detected]", ServerCore::getInstance()->getZoneServer()->getGalaxyName(), message, pvpNotificationChat->getRoomID());
								pvpNotificationChat->broadcastMessage(msg);
			...
			// Make sure that if fightclubbing was detected that we don't award anyone anything.
			if (isFightClubbing) {
				return;
			}
